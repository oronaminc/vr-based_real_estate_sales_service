...
public class HouseMapDaoImpl implements HouseMapDao {
	...
	public List<HouseInfoDto> getAptInDong(String dong, String type) throws Exception {
		...
		try {
			conn = DBUtil.getConnection();
			StringBuilder sql = new StringBuilder();
			sql.append("SELECT d.no, d.dong , AptName, d.code, d.jibun, dealAmount, rentMoney FROM houseinfo i right join housedeal d using (aptname) WHERE d.dong = ? and type = ?\n");
			...
			while(rs.next()) {
				HouseInfoDto dto = new HouseInfoDto();
				dto.setNo(rs.getString("d.no"));
				dto.setDong(rs.getString("d.dong"));
				dto.setAptName(rs.getString("AptName"));
				dto.setCode(rs.getString("d.code"));
				dto.setJibun(rs.getString("d.jibun"));
